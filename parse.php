<?php
require_once('excel_reader/excel_reader.php');     // include the class

date_default_timezone_set('UTC');

// creates an object instance of the class, and read the excel file data
$excel = new PhpExcelReader;
$excel->read('wetransfer-90fe78/BMNHN_S002_1961_T033_N001.xls');
...
function sheetData($sheet) {
  ...
  while($x <= $sheet['numRows']) {
    ...
    while($y <= $sheet['numCols']) {
      $cell = isset($sheet['cells'][$x][$y]) ? utf8_encode($sheet['cells'][$x][$y]) : '';
      $re .= " <td>$cell</td>\n";  
      $y++;
    }  
    $re .= "</tr>\n";
